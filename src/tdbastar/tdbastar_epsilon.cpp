...
// forward search, not clear how to with reverse search
// assumes double integrator_3d dynamics (pos, vel) for the residual force
// computation
bool check_lazy_trajectory_heterogeneous(
  ...
  // check for residual force with neighbors
  size_t max_t = 0;
  bool reachesGoal = false;
  if (robot.distance(tmp_traj.get_state(0), goal) <=
      delta) { // reaches the goal
    reachesGoal = true;
    for (const auto &sol : solution) {
      if (!sol.trajectory.states.empty())
        max_t = std::max(max_t, sol.trajectory.states.size() - 1);
  ...
  Eigen::VectorXd state1, state2;
  size_t primitive_starting_index =
      std::lround(best_node_gScore / all_robots[current_robot_idx]->ref_dt);
  max_t = std::max(max_t, primitive_starting_index + tmp_traj.get_size());
  for (size_t t = primitive_starting_index; t <= max_t; t++) {
    if (t - primitive_starting_index >= tmp_traj.get_size()) {
      state1 = tmp_traj.get_state(tmp_traj.get_size() - 1);
    } else {
      state1 = tmp_traj.get_state(t - primitive_starting_index);
    }
    // loop over all neighbors for the residual force at this timestamp t
    robot_idx = 0;
    rho = 0;
    for (auto &sol : solution) {
      if (robot_idx != current_robot_idx && !sol.trajectory.states.empty()) {
        ...
        // use nn for residual force prediction
        auto dist = state1 - state2; // only pos, velocity
    ...
    if (rho > max_f || rho < -max_f) {
      std::cout << "motion invalid, rho, time: " << rho << ", " << t
                << std::endl;
      motion_valid = false;
      return motion_valid;
    }
...
void tdbastar_epsilon(
  ...
  while (!stop_search()) {
    ...
    if (is_at_goal_no_constraints) {
      ...
      // current_arrival_idx doesn't point to any useful index/info.
