...
void Options_trajopt::add_options(po::options_description &desc) {
  ...
  set_from_boostop(desc, VAR_WITH_NAME(mim_line_search));
  set_from_boostop(desc, VAR_WITH_NAME(mim_mu));
...
void Options_trajopt::__read_from_node(const YAML::Node &node) {
  ...
  set_from_yaml(node, VAR_WITH_NAME(mim_line_search));
  set_from_yaml(node, VAR_WITH_NAME(mim_mu));
}
...
void Options_trajopt::print(std::ostream &out, const std::string &be,
  ...
  out << be << STR(mim_line_search, af) << std::endl;
  out << be << STR(mim_mu, af) << std::endl;
}
